...
describe('stashback', function () {
  ...
  it('should unstash callbacks', function () {
    async.waterfall([stash('key', callback, {}), unstash('key', {})], function (err, unstashed) {
      assert.ifError(err);
  ...
  it('should unstash all callbacks', function () {
    async.waterfall([stash('key1', callback, {}), stash('key2', callback, {}), stash('key3', callback, {}), unstashAll({})], function (err, unstashed) {
      assert.ifError(err);
  ...
  it('should unstash an unknown key', function () {
    async.waterfall([unstash('key', {})], function (err, unstashed) {
      assert.ok(err);
